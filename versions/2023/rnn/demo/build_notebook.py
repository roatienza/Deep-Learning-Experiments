"""Build the improved RNN MNIST notebook.

Regenerates rnn_mnist.ipynb from the cell definitions below.
...
# ---------------------------------------------------------------------------
md("""## MNIST Image Classification with an RNN Model

A Recurrent Neural Network (RNN) processes data **sequentially**: it reads
one element at a time, updating a hidden state that carries information
forward through the sequence. To classify an image with an RNN we treat the
28x28 image as a sequence of 28 rows, each row a 28-dimensional vector, and
read the rows top to bottom. After the last row, the hidden state summarizes
the whole image and a linear layer turns it into class logits.

This is a deliberately awkward fit for images (rows are not as naturally
ordered as words in a sentence), which makes the RNN a useful *baseline* to
compare against the MLP and CNN: it shows what you get from sequence
modeling alone, and why convolution is the better inductive bias for pixels.

In this notebook we:

1. Load and prepare the MNIST dataset (train / validation / test splits).
2. Visualize a few samples.
3. Define an RNN (hidden size 256) in PyTorch.
...
md("""### 3. Build the RNN model

* **Input:** a 28x28 image is reshaped to a sequence of 28 steps, each a
  28-dimensional vector (one per image row).
* **RNN:** one vanilla `nn.RNN` layer with hidden size 256,
  `batch_first=True`. At each step the hidden state is updated with
  `h = tanh(W_xh x_t + W_hh h_{t-1} + b)`.
* **Output:** after the last step, the final hidden state `h[:, -1, :]`
  (shape `(B, 256)`) is passed through a linear layer to produce 10 class
  logits. We output raw logits (no softmax) because `nn.CrossEntropyLoss`
  applies the log-softmax itself.

The model returns both the logits and the hidden states so you can inspect
the hidden trajectory if you like.
""")
...
class RNN(nn.Module):
    ...
    def forward(self, x):
        # x: (B, 1, 28, 28) -> sequence of 28 rows, each 28 features
        x = x.squeeze(1)                      # (B, 28, 28)
        y, h = self.rnn(x)                    # y: (B, 28, H), h: (L, B, H)
        logits = self.fc(y[:, -1, :])         # (B, 10)
        return logits, h
...
checkpoint = torch.load('rnn_mnist_best.pt', map_location=device)
...
This cell is **self-contained**: it reloads `rnn_mnist_best.pt` from disk and
...
Red panels typically reveal the confusions visible in the confusion matrix.
""")
...
| Item | Value |
|---|---|
| Architecture | RNN: 1 layer, hidden size 256, 28 steps of 28 features, linear head (256 -> 10) |
| Parameters | ~76k |
...
The RNN treats each image as a sequence of 28 rows and classifies from the
final hidden state. It is a weaker fit for images than the CNN (no weight
sharing across space, and row order is arbitrary), and it lands below the CNN (98.87% test) but above the
MLP baseline (97.83% test) at 97.69% test — which is exactly the point of
running it as a comparison. Section 7.5 lets you visually inspect what the
checkpoint predicts on fresh test images.
""")
