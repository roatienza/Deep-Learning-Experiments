"""Build the improved mamba_simple_mnist.ipynb (18 cells).
...
# ------------------------------------------------------------------ 0 intro
md("""## MNIST Image Classification with a Simple Mamba Model

Mamba is a state-space model (SSM) that processes sequences in linear
 time, like an RNN, but with a *selective* data-dependent transition
matrix that lets it route information adaptively. This "simple" variant
is the original Mamba (v1) kernel. For images we treat the picture as a
sequence of *patches*: a 28x28 image is split into a 4x4 grid of 7x7
patches (16 tokens, each 49 pixels), which two Mamba blocks process in
sequence order, and a linear head maps the flattened output (16 x 49 =
784) to 10 class logits.
...
3. Define a small patch-based Mamba model in PyTorch.
...
two `Mamba(d_model=49, expand=12)` blocks -> flatten (16 x 49)
...
Expected result: **~98% test accuracy** for this small Mamba model on MNIST.
""")
...
* **Two Mamba blocks** with `d_model=49` and `expand=12` (the inner
  state dimension is `d_model * expand = 588`). The selective SSM core
  computes a data-dependent transition for every token, which is what
  distinguishes Mamba from a vanilla RNN.
...
class MambaSimple(nn.Module):
    # Patch-based Mamba for MNIST: 4x4 patches -> 2 Mamba blocks -> head.
...
checkpoint = torch.load('mamba_simple_mnist_best.pt', map_location=device)
...
This cell is **self-contained**: it reloads `mamba_simple_mnist_best.pt` from
...
| Item | Value |
|---|---|
| Architecture | Patch Mamba: 4x4 patches (7x7) -> 2 Mamba blocks (d_model 49, expand 12) -> Linear(784, 10) |
| Parameters | ~150k |
...
Mamba processes the 16-patch sequence in linear time with a selective,
data-dependent state-space transition. Section 7.5 lets you visually
inspect what the checkpoint predicts on fresh test images.
""")
