"""Build the improved mamba2_mnist.ipynb (18 cells).
...
# ------------------------------------------------------------------ 0 intro
md("""## MNIST Image Classification with a Mamba2 Model

Mamba is a state-space model (SSM) that processes sequences in linear time,
like an RNN, but with a data-dependent transition matrix that lets it
selectively route information. Mamba2 is the newer, faster variant that
replaces the original selective SSM core with a hardware-aware parallel
scan. For images we treat the picture as a sequence of *patches*: a 28x28
image is split into a 4x4 grid of 7x7 patches (16 tokens, each 49 pixels),
which two Mamba2 blocks process in sequence order, and a linear head maps
the flattened output (16 x 49 = 784) to 10 class logits.

In this notebook we:

1. Install `mamba-ssm` and load the MNIST dataset (train / validation /
   test splits).
2. Visualize a few samples.
3. Define a small patch-based Mamba2 model in PyTorch.
...
two `Mamba2(d_model=49, headdim=49, expand=8)` blocks -> flatten (16 x 49)
...
Expected result: **~98% test accuracy** for this small Mamba2 model on MNIST.
""")

# ------------------------------------------------------------------ 1 install
code("""!pip install mamba-ssm[causal-conv1d] --upgrade""")
...
input (1 x 28 x 28)
  ...
  -> Mamba2(d_model=49, headdim=49, expand=8)   x2
  -> flatten (16 x 49 = 784)
...
* **Two Mamba2 blocks** with `d_model=49`, `headdim=49`, `expand=8`. The
  head count is `d_model * expand / headdim = 8`, which must be a multiple
  of 8 for the Mamba2 kernel (see
  https://github.com/state-spaces/mamba/issues/351).
...
class Mamba2Model(nn.Module):
    # Patch-based Mamba2 for MNIST: 4x4 patches -> 2 Mamba2 blocks -> head.
    ...
    def __init__(self, dim=49, num_classes=10):
        ...
        self.blocks = nn.ModuleList([
            Mamba2(
                d_model=dim,
                headdim=dim,
                expand=8,  # d_model * expand / headdim = 8 (multiple of 8)
            )
            for _ in range(2)
        ])
...
checkpoint = torch.load('mamba2_mnist_best.pt', map_location=device)
...
This cell is **self-contained**: it reloads `mamba2_mnist_best.pt` from
...
| Item | Value |
|---|---|
| Architecture | Patch Mamba2: 4x4 patches (7x7) -> 2 Mamba2 blocks (d_model 49, expand 8) -> Linear(784, 10) |
| Parameters | ~150k |
...
Mamba2 processes the 16-patch sequence in linear time with a data-dependent
state-space transition, which gives it sequence modeling with RNN-like
efficiency. Section 7.5 lets you visually inspect what the checkpoint
